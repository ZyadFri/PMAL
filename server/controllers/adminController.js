...
const getAllUsers = async (req, res) => {
  try {
    ...
    const filter = {};
    if (search) {
      filter.$or = [ { firstName: { $regex: search, $options: 'i' } }, { lastName: { $regex: search, $options: 'i' } }, { username: { $regex: search, $options: 'i' } }, { email: { $regex: search, $options: 'i' } } ];
    }
    ...
    const users = await User.find(filter).select('-password').sort(options.sort).limit(options.limit * 1).skip((options.page - 1) * options.limit);
...
const getUserProjects = async (req, res) => {
  try {
    ...
    const projects = await Project.find({ $or: [{ owner: id }, { manager: id }, { 'team.user': id }] }).populate('owner', 'firstName lastName email').populate('manager', 'firstName lastName email').sort({ updatedAt: -1 });

    res.json({
      success: true,
      user: { id: user._id, firstName: user.firstName, lastName: user.lastName, email: user.email, username: user.username },
      projects: projects
    });
  } catch (error) {
    console.error('Get user projects error:', error);
    res.status(500).json({ success: false, message: 'Server error while fetching user projects', error: error.message });
  }
};
