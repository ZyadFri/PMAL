...
const getProjects = async (req, res) => {
  try {
    ...
    // Build filter object
    const filter = {
      $or: [{ owner: userId }, { manager: userId }, { "team.user": userId }],
      isActive: true,
    };
  ...
  try {
    ...
    if (!assessment) {
      return res
        ...
        .json({
          success: false,
          message: "No completed assessment found for this project.",
        });
    }
    ...
    doc
      .fontSize(24)
      .font("Helvetica-Bold")
      .text("Project Assessment Report", { align: "center" });
    ...
    assessment.categoryScores.forEach((cs) => {
      if (cs.category) {
        // Safeguard
        const avgScore =
          cs.questionsAnswered > 0 ? cs.score / cs.questionsAnswered : 0;
    ...
    if (assessment.feedback.nextSteps.length > 0) {
      ...
      assessment.feedback.nextSteps.forEach((s) =>
        doc.font("Helvetica").list([s])
      );
    }
    ...
    const kpis = {
      ...
      lowestScore:
        assessments.length > 0
          ? Math.min(...assessments.map((a) => a.overallScore))
          : 0,
      averageScore:
        assessments.length > 0
  ...
  try {
    ...
    const averageScore =
      projectsWithScores.length > 0
        ? projectsWithScores.reduce(
            (sum, project) => sum + project.overallScore,
            0
